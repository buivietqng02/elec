define([
    ...
    const getCurrentTime = (today) => `${((today.getHours() < 10) ? '0' : '')}${today.getHours()}${((today.getMinutes() < 10) ? '0' : '')}${today.getMinutes()}${((today.getSeconds() < 10) ? '0' : '')}${today.getSeconds()}`;
    ...
    const converseDDMMYYY = (date) => {
        ...
        return `${converseDate.hh}:${converseDate.mm}:${converseDate.ss} - ${converseDate.day}/${window.parseInt(converseDate.month) + 1}/${converseDate.year}`;
    };
    ...
    const getParamEPAY = async (addAmount, payMethod) => {
        ...
        const sha256Reponse = await sha256(`${timeStamp}${merTrxId}${config.MER_ID}${amount}${config.ENCODE_KEY}`);
